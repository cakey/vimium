const Commands = {
  ...
  loadKeyMappings(customKeyMappings) {
    ...
    // mode.
    const passNextKeys = Object.keys(this.keyToCommandRegistry).filter(
      (key) =>
        this.keyToCommandRegistry[key].command === "passNextKey" &&
        key.length > 1
    );
...
const defaultKeyMappings = {
  ...
  d: "LinkHints.activateModeWithQueue",
...
const commandDescriptions = {
  ...
  removeTab: [
    "Close current tab",
    {
      background: true,
      repeatLimit:
        (chrome.session ? chrome.session.MAX_SESSION_RESULTS : null) || 25,
    },
  ],
